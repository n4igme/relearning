...
// Ambil soal dari database untuk kuis id = 3
$id_kuis = 3;
...
if ($totalQuestions === 0) {
    echo "Tidak ada soal ditemukan.";
    exit;
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $_SESSION['quiz3_answers'][$currentQuestion] = $_POST['answer'] ?? null;
...
<head>
    <meta charset="UTF-8">
    <title>Kuis Desain Grafis Lanjutan</title>
    ...
    <script>
        function validateForm() {
            ...
            if (!valid) {
                alert("Silakan pilih jawaban dulu.");
                return false;
            }
            return true;
        }
...
<div class="bg-white rounded-2xl shadow-xl w-full max-w-3xl p-8">
    ...
    <div class="flex justify-between items-center mb-8">
        <h1 class="text-2xl font-bold text-indigo-600">Kuis Desain Grafis Lanjutan</h1>
        <span class="text-gray-600"><?= ($currentQuestion + 1) ?> / <?= $totalQuestions ?></span>
    </div>

    <div class="w-full bg-gray-300 rounded-full h-2.5 mb-8">
        <div class="bg-indigo-500 h-2.5 rounded-full" style="width: <?= (($currentQuestion + 1) / $totalQuestions) * 100 ?>%"></div>
    </div>
    ...
    <form method="post" action="?q=<?= $currentQuestion ?>" onsubmit="return validateForm()" class="space-y-6">
            ...
            <?php foreach ($questions[$currentQuestion]['options'] as $key => $option): ?>
                <div class="flex items-center mb-4">
                    <input 
                        ...
                        <?= (isset($_SESSION['quiz3_answers'][$currentQuestion]) && $_SESSION['quiz3_answers'][$currentQuestion] === $key) ? 'checked' : '' ?>
                    >
        ...
        <div class="flex justify-between mt-8">
            ...
            <?php if ($currentQuestion < $totalQuestions - 1): ?>
                <button name="next" class="px-6 py-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600">
                    Selanjutnya
                </button>
            <?php else: ?>
